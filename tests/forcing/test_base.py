...
class TestFactoryFunction:
    ...
    def test_create_interpolated_forcing_with_method(self):
        ...
        forcing = create_forcing_function(
            "interpolated", times=[0, 1, 2], values=[5, 10, 15], interpolation_method="cubic"
        )
    ...
    def test_unknown_forcing_type_error(self):
        ...
        with pytest.raises(
            ValueError,
            match="Available:.*onoff.*perdose.*periodic.*ndoses.*multidose.*interpolated",
        ):
...
class TestBackendParametrizedFunctionality:
    ...
    def _is_numeric_result(self, result):
        ...
        try:
            ...
            if (
                hasattr(result, "__array__")
                or "jax" in str(type(result))
