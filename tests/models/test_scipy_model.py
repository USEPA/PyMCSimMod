...
class TestScipyModelForcingFunctions:
    ...
    def test_onoff_forcing_function(self, simple_scipy_model, standard_times, onoff_forcing_params):
        ...
        forcing_period_mask = (result.times >= onoff_forcing_params["t0"]) & (
            result.times <= onoff_forcing_params["t1"]
        )
    ...
    def test_perdose_forcing_function(self, simple_scipy_model, perdose_forcing_params):
        ...
        dose_times = [
            perdose_forcing_params["t0"] + i * perdose_forcing_params["period"] for i in range(3)
        ]
        for dose_time in dose_times:
            if dose_time < times[-1]:
                # Find times around dose administration
                mask = (times >= dose_time) & (
                    times <= dose_time + perdose_forcing_params["duration"]
                )
                if np.any(mask):
    ...
    def test_ndoses_forcing_function(self, simple_scipy_model, ndoses_forcing_params):
        ...
        for dose_time in ndoses_forcing_params["t0_list"]:
            if dose_time < times[-1]:
                ...
                mask = (times >= dose_time) & (
                    times <= dose_time + ndoses_forcing_params["duration"]
                )
                if np.any(mask):
    ...
    def test_interpolated_forcing_function(self, simple_scipy_model, interpolation_data):
        ...
        model.assign_forcing_function("dose", "InterpolatedForcing", data_dict=forcing_data)
    ...
    def test_forcing_function_switch_times(self, simple_scipy_model, onoff_forcing_params):
        ...
        assert onoff_forcing_params["t0"] in result.times or any(
            np.isclose(result.times, onoff_forcing_params["t0"])
        )
        assert onoff_forcing_params["t1"] in result.times or any(
            np.isclose(result.times, onoff_forcing_params["t1"])
        )
...
class TestScipyModelDiscreteEvents:
    ...
    def test_multiple_discrete_events(self, simple_scipy_model, sample_events):
        ...
        for event in sample_events:
            model.add_event(
                time=event.time, state_var=event.state_var, value=event.value, method=event.method
            )
    ...
    def test_events_at_boundaries(self, simple_scipy_model, short_times):
        ...
        model.add_event(
            time=short_times[len(short_times) // 2], state_var="A", value=15.0, method="replace"
        )
    ...
    def test_events_outside_time_range(self, simple_scipy_model, short_times):
        ...
        assert np.isclose(
            result.states[0, 0], 10.0, atol=0.1
        )  # Should start with initial condition
