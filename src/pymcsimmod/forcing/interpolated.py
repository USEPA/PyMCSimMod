...
class InterpolatedForcing(ForcingFunction):
    ...
    def plot_data(self, ax=None, show_points=True, show_interpolation=True, 
                  n_interp_points=100, **kwargs):
        """
        ...
        if show_points:
            ax.scatter(self.times, self.values, color='red', s=50, 
                      zorder=3, label='Data points', **kwargs)
        ...
        if show_interpolation:
            ...
            ax.plot(t_interp, v_interp, 'b-', linewidth=2, 
    ...
    def __repr__(self):
        ...
        return (f"InterpolatedForcing(n_points={len(self.times)}, "
...
def create_interpolated_forcing(
    data: Union[pd.DataFrame, dict, tuple],
